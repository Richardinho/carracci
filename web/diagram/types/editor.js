define(['BaseType', 'diagram/types/editorView', 'events/eventsBus'], function(
  BaseType,
  TypeEditorView,
  eventsBus
) {
  ...
  return BaseType.extend({
    initialize: function(options) {
      ...
      this.view.$el.on(
        'click',
        '[data-role=add-prop]',
        $.proxy(this.addProp, this)
      )
      this.view.$el.on(
        'click',
        '[data-role=add-method]',
        $.proxy(this.addMethod, this)
      )
      this.view.$el.on(
        'click',
        '[data-role=deleteprop]',
        $.proxy(this.deleteProp, this)
      )
      this.view.$el.on(
        'click',
        '[data-role=deletemethod]',
        $.proxy(this.deleteMethod, this)
      )
      this.view.$el.on(
        'click',
        '[data-role=methodArgs]',
        $.proxy(this.argsClick, this)
      )
      this.view.$el.on(
        'click',
        '[data-role=add-note]',
        $.proxy(this.addNote, this)
      )
    ...
    save: function() {
      ...
      $('[data-role=property-table] tbody tr', this.view.$el).each(
        function(index, row) {
      ...
      $('[data-role=method-table] tbody tr', this.view.$el).each(
        function(index, row) {
          if (that._validateRow(row)) {
            var name = $('input[name=name]', row).val()
    ...
    _validateRow: function(row) {
      var inputs = row.querySelectorAll(
        '[type=text]:not([data-role=methodArgs])'
      )
